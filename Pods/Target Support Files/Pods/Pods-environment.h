
// To check if a library is compiled with CocoaPods you
// can use the `COCOAPODS` macro definition which is
// defined in the xcconfigs so it is available in
// headers also when they are imported in the client
// project.


// Batch
#define COCOAPODS_POD_AVAILABLE_Batch
#define COCOAPODS_VERSION_MAJOR_Batch 1
#define COCOAPODS_VERSION_MINOR_Batch 2
#define COCOAPODS_VERSION_PATCH_Batch 3

// Bolts
#define COCOAPODS_POD_AVAILABLE_Bolts
#define COCOAPODS_VERSION_MAJOR_Bolts 1
#define COCOAPODS_VERSION_MINOR_Bolts 1
#define COCOAPODS_VERSION_PATCH_Bolts 4

// Branch
#define COCOAPODS_POD_AVAILABLE_Branch
#define COCOAPODS_VERSION_MAJOR_Branch 0
#define COCOAPODS_VERSION_MINOR_Branch 4
#define COCOAPODS_VERSION_PATCH_Branch 7

// DRDynamicSlideShow
#define COCOAPODS_POD_AVAILABLE_DRDynamicSlideShow
#define COCOAPODS_VERSION_MAJOR_DRDynamicSlideShow 1
#define COCOAPODS_VERSION_MINOR_DRDynamicSlideShow 0
#define COCOAPODS_VERSION_PATCH_DRDynamicSlideShow 1

// Heap
#define COCOAPODS_POD_AVAILABLE_Heap
#define COCOAPODS_VERSION_MAJOR_Heap 2
#define COCOAPODS_VERSION_MINOR_Heap 1
#define COCOAPODS_VERSION_PATCH_Heap 3

// JTSImageViewController
#define COCOAPODS_POD_AVAILABLE_JTSImageViewController
#define COCOAPODS_VERSION_MAJOR_JTSImageViewController 1
#define COCOAPODS_VERSION_MINOR_JTSImageViewController 4
#define COCOAPODS_VERSION_PATCH_JTSImageViewController 0

// Parse
#define COCOAPODS_POD_AVAILABLE_Parse
#define COCOAPODS_VERSION_MAJOR_Parse 1
#define COCOAPODS_VERSION_MINOR_Parse 7
#define COCOAPODS_VERSION_PATCH_Parse 1

// iRate
#define COCOAPODS_POD_AVAILABLE_iRate
#define COCOAPODS_VERSION_MAJOR_iRate 1
#define COCOAPODS_VERSION_MINOR_iRate 11
#define COCOAPODS_VERSION_PATCH_iRate 3

